...
export default function Trouver() {
  return (
    ...
    >
      ...
      <h2 className="m-0 mb-9 text-[clamp(34px,calc(5.2*var(--u)),96px)] leading-[1.06]">
        {trouver.titre1} <span className="blue">{trouver.titre2}</span>
      </h2>

      <p className="lede m-0 mb-14 max-w-none">{trouver.lede}</p>

      <p className="serif m-0 mb-[26px] text-[clamp(22px,calc(2.2*var(--u)),36px)] leading-[1.3]">
        {trouver.amorce}
      </p>

      <div className="cartes6 border-2 border-line" style={{ background: "#D8CFC0" }}>
        {trouver.cartes.map((carte, i) => (
          <article key={carte.titre} className="card">
            ...
            <h3 className="m-0 mb-4 font-[family-name:var(--font-display)] text-[clamp(21px,calc(1.8*var(--u)),30px)] leading-[1.12] uppercase">
              {carte.titre}
            </h3>
